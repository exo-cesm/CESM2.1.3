...
contains
  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    if( masterproc ) then
      write(iulog,*) 'photo_inti: H2O index', h2o_ndx
      write(iulog,*) 'photo_inti: CO2 index', co2_ndx
    endif
...
!-----------------------------------------------------------------
    ...
    real(r8), allocatable :: h2o_den(:)             ! h2o density (molecules/cm^3)
    real(r8), allocatable :: co2_den(:)             ! co2 density (molecules/cm^3)
    ...
    if (do_jshort) then
       ...
       allocate( h2o_den(n_jshrt_levs) )
       allocate( co2_den(n_jshrt_levs) )
...
!------------------------------------------------------------------------------------------------------------
    ...
    col_loop : do i = 1,ncol
       if (do_jshort) then
          ...
          h2o_den(p1:p2) = vmr(i,:pver,h2o_ndx) * invariants(i,:pver,indexm)
          co2_den(p1:p2) = vmr(i,:pver,co2_ndx) * invariants(i,:pver,indexm)
       ...
       daylight : if( sza >= 0._r8 .and. sza < max_zen_angle ) then
         ...
         if ( ptop_ref > 10._r8 ) then
          ...
          if (do_jshort) then
             ...
             !-----------------------------------------------------------------
             call jshort( n_jshrt_levs, sza, n2_den, o2_den, o3_den, no_den, &
                          h2o_den, co2_den, tline, zarg, jo2_sht, jno_sht, sht_prates )  
    ...
    if ( allocated(h2o_den) ) deallocate( h2o_den )
    if ( allocated(co2_den) ) deallocate( co2_den )
