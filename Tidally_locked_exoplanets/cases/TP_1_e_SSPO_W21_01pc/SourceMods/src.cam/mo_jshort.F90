...
!------------------------------------------------------------------------------
      ...
      real(r8) :: xs_co2srb(nsrb_tot)
      real(r8) :: xs_h2osrb(nsrb_tot)
      ...
      real(r8), allocatable :: xs_tmp(:)  ! temporary buffer to read in xs
...
!------------------------------------------------------------------------------
	 ...
	 call alloc_err( ierr, 'get_crs', 'xs_o2src', nw )
      ...
      allocate( xs_tmp(nw),stat=ierr )
      if( ierr /= 0 ) then 
         call alloc_err( ierr, 'get_crs', 'xs_tmp', nw )
      end if
...
!------------------------------------------------------------------------------
      ...
      !------------------------------------------------------------------------------
      !       ... get co2 and h2o cross sections
      !------------------------------------------------------------------------------
      ierr = pio_inq_varid( ncid, 'jco2', varid )
      ierr = pio_get_var( ncid, varid, xs_tmp )
      xs_co2srb(:) = xs_tmp(nsrc_tot+1:nsrc_tot+nsrb_tot)

      ! sum 3 H2O cross-sections in the SRB
      ierr = pio_inq_varid( ncid, 'jh2o_a', varid )
      ierr = pio_get_var( ncid, varid, xs_tmp )
      xs_h2osrb(:) = xs_tmp(nsrc_tot+1:nsrc_tot+nsrb_tot)

      ierr = pio_inq_varid( ncid, 'jh2o_b', varid )
      ierr = pio_get_var( ncid, varid, xs_tmp )
      xs_h2osrb(:) = xs_h2osrb(:) + xs_tmp(nsrc_tot+1:nsrc_tot+nsrb_tot)

      ierr = pio_inq_varid( ncid, 'jh2o_c', varid )
      ierr = pio_get_var( ncid, varid, xs_tmp )
      xs_h2osrb(:) = xs_h2osrb(:) + xs_tmp(nsrc_tot+1:nsrc_tot+nsrb_tot)
      !------------------------------------------------------------------------------
...
!-------------------------------------------------------------
! Print out cross sections for CO2 and H20 used in the 14 SRB bands
!-------------------------------------------------------------
      if (masterproc) then
         write(iulog,*) 'get_crs : SRB co2'
         do m = 1,nsrb_tot
           write(iulog,*) 'SRB co2', &
                          m, wc(nsrc_tot+m), xs_co2srb(m)
         enddo
         write(iulog,*) 'get_crs : SRB h2o'
         do m = 1,nsrb_tot
           write(iulog,*) 'SRB h2o', &
                          m, wc(nsrc_tot+m), xs_h2osrb(m)
         enddo
      endif
...
!------------------------------------------------------------------------------
	 ...
	 k1       = nlev - k + 1
      ...
      subroutine jshort_photo( nlev, zen, n2cc, o2cc, o3cc, nocc, &
                               h2occ, co2cc, tlev, zkm, jo2_sht, jno_sht, jsht )
!==============================================================================!
!   Subroutine Jshort                                                          !
!                                                                              !
!==============================================================================!
...
!	h2occ...... H2O concentration, molecule cm-3			       !
!	co2cc...... CO2 concentration, molecule cm-3			       !
!	n2cc....... N2 concentration, molecule cm-3			       !
!     zen........ zenith angle, units = degrees                                !
!     tlev....... Temperature Profile (K)                                      !
!     zkm ....... Altitude, km                                                 !
...
!------------------------------------------------------------------------------
	...
	real(r8), intent(in)    :: o3cc(nlev)		! Ozone concentration (mol/cm^3)
        ...
        real(r8), intent(in)    :: h2occ(nlev)		! Water vapor conc (mol/cm^3)
        real(r8), intent(in)    :: co2cc(nlev)		! Water vapor conc (mol/cm^3)
	real(r8), intent(in)    :: tlev(nlev)		! Temperature profile
	real(r8), intent(in)    :: zkm(nlev)		! Altitude, km
...
!------------------------------------------------------------------------------
	...
	real(r8) :: o2scol(nlev)             ! O2 Slant Column
        real(r8) :: o3scol(nlev)	     ! O3 Slant Column
        real(r8) :: co2scol(nlev)	     ! CO2 Slant Column
        real(r8) :: h2oscol(nlev)	     ! H2O Slant Column
	...
	real(r8) :: abs_col(nlev)
        real(r8) :: tsrb(nlev,nsrbtuv)       ! Transmission in the SRB
        real(r8) :: tsrb_h2o(nlev,nsrbtuv)   ! Transmission in the SRB due to H2O
        real(r8) :: tsrb_co2(nlev,nsrbtuv)   ! Transmission in the SRB due to CO2
	real(r8) :: xs_o2srb(nlev,nsrbtuv)   ! Cross section * QY for O2 in SRB
...
!------------------------------------------------------------------------------
!     ... Derive O2, O3, CO2, H2O and NO Slant Column
!------------------------------------------------------------------------------
      delz(1:nlev-1) = km2cm*(zkm(1:nlev-1) - zkm(2:nlev))
      call slant_col( nlev, delz, dsdh, nid, o2cc, o2scol )
      call slant_col( nlev, delz, dsdh, nid, o3cc, o3scol )
      call slant_col( nlev, delz, dsdh, nid, nocc, noscol )
      call slant_col( nlev, delz, dsdh, nid, h2occ, h2oscol )
      call slant_col( nlev, delz, dsdh, nid, co2cc, co2scol )
...
!------------------------------------------------------------------------------
!  	... transmission due to H2O and CO2 in the SRB
!------------------------------------------------------------------------------
      do wn = 1,nsrb_tot
         abs_col(:) = min( xs_h2osrb(wn)*h2oscol(:),100._r8 )
         tsrb_h2o(:,wn) = exp( -abs_col(:) )
         abs_col(:) = min( xs_co2srb(wn)*co2scol(:),100._r8 )
         tsrb_co2(:,wn) = exp( -abs_col(:) ) 
      end do
...
!------------------------------------------------------------------------------
      do wn = 1,nsrb_tot
         trans_o2(:,wn+nsrc_tot) = tsrb(:,wn)* tsrb_h2o(:,wn) * tsrb_co2(:,wn)
      end do
